...
type GenerateChannelOptions = {
  channel: Partial<Channel>
};

export const generateChannel = (options: GenerateChannelOptions = { channel: {} }) => {
  const { channel: optionsChannel, ...optionsBesidesChannel } = options;
  const id = (optionsChannel && optionsChannel.id) || generateRandomId();
